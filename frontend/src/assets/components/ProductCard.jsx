...
async function updateProductById(id) {
  ...
  if (response.ok) {
    ...
    console.log(data);
    console.log("Product upadted succefully");
    refreshProducts();
  ...
  return (
      <div 
        key={key}
        className=" border border-gray-500/25 bg-gray-950   rounded-2xl  shadow-lg"
      >
        <div>
          <img
            src={product?.imageUrl}
            alt={product?.name}
            className="scale-100 overflow-hidden rounded-2xl h-70 w-full"
          />
          <div className="p-1">
            <h4>{product?.name}</h4>
            <p>${product?.price}</p>
